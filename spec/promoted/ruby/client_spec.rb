...
RSpec.describe Promoted::Ruby::Client::PromotedClient do
  ...
  context "prepare_for_logging when no limit is set" do
    ...
    it "should have insertion set" do
      client = described_class.new ENDPOINTS
      logging_json = client.prepare_for_logging(input)
      expect(logging_json[:delivery_log][:response][:insertion].length).to eq(input[:full_insertion].length)
      expect(logging_json[:delivery_log][:response][:insertion]).not_to be nil
    end

    it "should have request_id set since insertions aren't coming from delivery" do
      client = described_class.new ENDPOINTS
      logging_json = client.prepare_for_logging(input)
      expect(logging_json[:delivery_log].key?(:request)).to be true
      expect(logging_json[:delivery_log][:request][:request_id]).not_to be nil
      logging_json[:delivery_log][:response][:insertion].each do |insertion|
  ...
  context "prepare_for_logging when no limit is set" do
    ...
    it "should have insertion set" do
      ...
      logging_json = client.prepare_for_logging(input)
      expect(logging_json[:delivery_log][:response][:insertion]).not_to be nil
      expect(logging_json[:delivery_log][:response][:insertion].length).to eq(input_with_limit[:request].dig(:paging, :size).to_i)
    end
  end
  ...
  context "extra fields at the top level on insertions" do
    ...
    it "passes along extra fields on the insertions" do
      ...
      client = described_class.new ENDPOINTS
      logging_json = client.prepare_for_logging(dup_input)
      expect(logging_json[:delivery_log][:response][:insertion][0].key?(:session_id)).to be true
      expect(logging_json[:delivery_log][:response][:insertion][0][:session_id]).to eq "uuid0"
      expect(logging_json[:delivery_log][:response][:insertion][0].key?(:view_id)).to be true
      expect(logging_json[:delivery_log][:response][:insertion][0][:view_id]).to eq "uuid0"
    end
  end
  ...
  context "deliver" do
    ...
    it "does not deliver for request only_log" do
      ...
      expect(deliver_resp[:client_request_id]).to eq(deliver_resp[:log_request][:delivery_log][:request][:client_request_id])
    end
    ...
    it "swallows errors and defaults the insertions" do
      ...
      expect(log_request[:delivery_log][:request][:client_request_id]).to eq(delivery_req[:client_request_id])
      expect(deliver_resp[:client_request_id]).to eq(delivery_req[:client_request_id])
  ...
  context "cohorts" do
    ...
    it "delivers shadow traffic for control arm by default" do
      ...
      expect(deliver_resp[:log_request][:delivery_log].key?(:response)).to be true
      expect(deliver_resp[:log_request][:delivery_log][:response].key?(:insertion)).to be true
      expect(deliver_resp[:log_request][:delivery_log].key?(:request)).to be true
      ...
      expect(logging_json[:delivery_log].key?(:request)).to be true
      expect(logging_json[:delivery_log][:request][:client_request_id]).not_to be nil
      expect(logging_json[:delivery_log][:request][:request_id]).not_to be nil
      logging_json[:delivery_log][:response][:insertion].each do |insertion|
        expect(insertion[:request_id]).not_to be nil
      end
      expect(deliver_resp[:client_request_id]).to eq(logging_json[:delivery_log][:request][:client_request_id])
